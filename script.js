...
conciergeToggle.addEventListener('click', () => {
  if (conciergePanel.hidden) openConciergePanel();
  else closeConciergePanel();
});

conciergeClose.addEventListener('click', closeConciergePanel);
panelScrim.addEventListener('click', closeConciergePanel);
document.addEventListener('keydown', (event) => {
  if (event.key === 'Escape' && !conciergePanel.hidden) closeConciergePanel();
});
